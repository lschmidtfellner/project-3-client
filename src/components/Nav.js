  ...
  return (
    <div>
      ...
      <div style={{display: isMenuOpen ? 'block' : 'none'}} className='menu bg-off-white fixed top-[90px] py-10 leading-10 right-0 left-0 text-center text-2xl border-b border-black'>
          ...
          {loggedIn ? <h2 onClick={() => handleSignout()} >Sign Out</h2> : <h2 onClick={() => handleNavigate('/auth/signin')}>Sign In</h2>}
        </div>
      <div style={{ top: isNavVisible ? '0' : '-100px', transition: 'top 0.3s' }} className="fixed bg-off-white border-b border-black flex items-center nav h-[90px] w-full">
